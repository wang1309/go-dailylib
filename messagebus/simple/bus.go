...
func (m *messageBus) Subscribe(topic string, fn interface{}) error {
	...
	h := &handler{
		...
		queue:    make(chan []reflect.Value, m.handlerQueueSize),
	}

	// 启动一个goroutine，监听通道，每当有参数到来时就执行注册的回调
	go func() {
...
func (m *messageBus) Publish(topic string, args ...interface{}) {
